...
## Session log
- 2026-08-07: Ported backend PR #407 (supplier profiles + vendor fixes): new `SupplierDetailNativeView` in `TireShop/SupplierScreens.swift` — supplier profile with 6 summary stats (`GET /suppliers/:id`), info grid, and four paginated tabs (Containers/Bills/Payments/Warranty claims) via new `SuppliersAPI.containers/costs/payments/returns`; Bills tab has scope + status filters; payments open the shared `SupplierPaymentDetailSheet`; supplier rows in Purchasing now navigate to the profile. **Vendor detail** updated for the breaking response change: `VendorDetail` dropped `recentCosts/recentExpenses/recentRefunds` (now `summary` only) and the three activity lists load from the new paginated `VendorsAPI.costs/expenses/refunds` with per-list paging. New models `SupplierSummary`/`SupplierDetail`/`SupplierContainerRow`/`SupplierCostRow`/`SupplierPaymentRow`/`SupplierReturnRow`; `AppRoute.supplierDetail` added. Regenerated `TireShop.xcodeproj`; verifier green, `xcodebuild` build + full test suite passed on iPhone 17 simulator.
