...
- 2026-06-30: Added **Vendors** native module from `vendors/page.tsx` and `[id]/page.tsx`: list filters, create/edit/deactivate, spend summary, recent costs/expenses, refund history, refund record, and refund reverse. Verifier green: 21 swift files, 28 built destinations. `xcodebuild` succeeded on iPhone 17 simulator. Employees is in draft PR #2 on a separate branch; remaining Phase A after both land: Customer Relations (CRM).
- 2026-06-30: Added **Employees** native module from `employees/page.tsx`, `[id]/page.tsx`, and `EmployeeModal.tsx`: list search/status filtering, create/edit sheet, detail profile/compensation, linked user picker, commission summary, recent commissions, payout history, and payout action. Verifier green: 21 swift files, 28 built destinations. `xcodebuild` succeeded on iPhone 17 simulator. Next remaining Phase A: Vendors, Customer Relations (CRM).
